...
export default function Scanner() {
  ...
  useEffect(() => {
    const load = () =>
      listTargets({ limit: 200000, offset: 0 }).then((r) =>
        setResults(r.rows),
      );
    ...
    <div className="space-y-6">
      ...
      <GlassCard title="Scan configuration">
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              onClick={() => scanner.start(scanCount)}
              disabled={progress?.running}
            >
      ...
      <div className="grid md:grid-cols-4 gap-4">
        ...
        <Metric
          ...
          value={
            progress?.eta
              ? `${Math.max(0, Math.round(progress.eta))}s`
              : "—"
          }
        />
      ...
      <GlassCard title="Live progress">
        ...
        <div className="flex justify-between text-xs text-slate-400 mt-2">
          ...
          <span>
            {progress?.online ?? 0} online ·{" "}
            {progress?.offline ?? 0} offline
          </span>
